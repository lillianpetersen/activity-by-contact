...
n,bins,patches = axs[0].hist(abcConnected, bins=logbins, color='lime', alpha=0.9, label='Tested, p<0.05')
axs[0].plot([np.median(abcConnected),np.median(abcConnected)],[0,np.amax(n)],'k',linewidth=2)
axs[0].grid(True)
axs[0].legend(loc='upper left',fontsize=12)
plt.xscale('log')

n,bins,patches = axs[1].hist(abcNotConnected, bins=logbins, color='b', alpha=0.9, label='Tested, p>0.2')
...
plt.savefig(wdfigs+'validation_K562/known_unknown_abc_distributions.pdf')
plt.show()
...
plt.clf()
fig, axs = plt.subplots(3, 1, sharex=True, sharey=False)
n,bins,patches = axs[0].hist(atacContactConnected, bins=logbins, color='lime', alpha=0.9, label='Tested, p<0.05')
axs[0].plot([np.median(atacContactConnected),np.median(atacContactConnected)],[0,np.amax(n)],'k',linewidth=2)
axs[0].grid(True)
axs[0].legend(loc='upper left',fontsize=12)
plt.xscale('log')

n,bins,patches = axs[1].hist(atacContactNotConnected, bins=logbins, color='b', alpha=0.9, label='Tested, p>0.2')
...
plt.ylabel('Frequency')
plt.xlabel('ABC (no H3K27ac) Score, log scale')
plt.xscale('log')
plt.grid(True)
plt.legend(loc='upper left',fontsize=12)
fig.suptitle('ABC (no H3K27ac) Score of Enhancer-Gene Connections\nT-test = '+str(np.round(TatacContact,2))+', P = '+str(np.format_float_scientific(PatacContact,1)),fontsize=17)
...
plt.clf()
fig, axs = plt.subplots(3, 1, sharex=True, sharey=False)
n,bins,patches = axs[0].hist(distConnected, bins=logbins, color='lime', alpha=0.9, label='Tested, p<0.05')
axs[0].plot([np.median(distConnected),np.median(distConnected)],[0,np.amax(n)],'k',linewidth=2)
axs[0].grid(True)
plt.xscale('log')
axs[0].legend(loc='upper left',fontsize=12)

n,bins,patches = axs[1].hist(distNotConnected, bins=logbins, color='b', alpha=0.8, label='Tested, p>0.2')
...
plt.plot(100*recallAtacContact,100*precisionAtacContact,'g-',linewidth=3,label='ABC (no H3K27ac)')
...
plt.plot(fprAtacContact, tprAtacContact, color='g', lw=3, label='ABC (No H3K27ac)')
